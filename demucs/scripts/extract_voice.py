...
def process_songs():
    ...
    if not mp3_files:
        print(f"No MP3 files found in {data_folder} folder")
        return
    ...
    for i, mp3_file in enumerate(mp3_files, 1):
        ...
        try:
            ...
            # Save vocal file (updated path)
            vocal_path = Path("../separated/01_demucs/voice") / f"voice_{song_name}.wav"
            sf.write(str(vocal_path), vocals.T, sample_rate)
            print(f"  ✅ Saved: voice_{song_name}.wav")
            
            # Save instrumental file (updated path)
            instrumental_path = Path("../separated/01_demucs/instrumental") / f"instrumental_{song_name}.wav"
...
def setup_directories():
    ...
    os.makedirs("../separated/01_demucs/voice", exist_ok=True)
    os.makedirs("../separated/01_demucs/instrumental", exist_ok=True)
